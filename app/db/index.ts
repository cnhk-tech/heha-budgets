  ...
  return new Promise((resolve) => {
    ...
    request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
      ...
      if (oldVersion > 0 && oldVersion < 3) {
        ...
        const addUserReq = userStore.add({ name: 'User', currency: 'USD' });
        ...
        // Fresh install: empty Categories and Budgets stores only — no default user or seed data
    ...
    request.onsuccess = () => {
      registerDbConnection(request.result);
      setDBReady(true);
      resolve(true);
    };
